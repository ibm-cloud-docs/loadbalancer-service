...
The following section provides information on common error messages that you might encounter when you create or update an instance of {{site.data.keyword.loadbalancer_full}}.
{: shortdesc}
...
|`Specified private subnet VLAN is on router 'xyz'. However, no public subnet with 'n' free IPs was found with VLAN on the same router.`|This issue happens because you selected the **Allocate from public subnet from this account** option during provisioning.|Select **Allocate from IBM System Pool** instead, or contact IBM Support.|
...
|`There was a problem fetching the servers.`|This issue can happen as a result of network timeout issues.|Reload the page. If the issue persists, contact IBM Support.|
