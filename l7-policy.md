...
The `Redirect to https` policy is evaluated after a `Reject`. If this policy exists, then any existing `Redirect to url` and `Redirect to pool` policies do not apply. Also, if any existing `Redirect to https` policies exist, then you can't create any new `Redirect to url` and `Redirect to pool` policies.
