...
Gli algoritmi di cifratura di supporto a {{site.data.keyword.loadbalancer_full}} stabiliscono connessioni sicure con i suoi client HTTP.
...
Per scegliere una configurazione della suite di cifratura per un programma di bilanciamento del carico esistente, vai alla schermata del tuo programma di bilanciamento del carico nel portale clienti e fai clic sulla scheda relativa ai protocolli. Se HTTPS non è selezionato come protocollo di frontend, non vedrai l'elenco delle suite di cifratura.
...
Per un elenco delle cifrature supportate, vedi [Offload SSL](/docs/infrastructure/loadbalancer-service?topic=loadbalancer-service-ssl-offload-with-ibm-cloud-load-balancer).
{: note}
