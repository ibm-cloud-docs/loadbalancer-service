...
The back-end compute instances that host your application must also be on the IBM Cloud Private network.
{: note}
...
After the load balancer is provisioned, only the public IP addresses of the members in the server tables context are visible. This behavior is applicable for both layer 4 and layer 7 load balancer members.
...
The back-end compute instances that host your application must be on the IBM Cloud public network.
{: note}
...
The supported front-end ports and protocols are HTTP, HTTPS, and TCP. The supported back-end ports/protocols are also HTTP, HTTPS, and TCP. Incoming HTTPS traffic must be terminated at the load balancer to allow for plain text HTTP communication with the back-end server. If the back-end protocol is HTTPS, the traffic is encrypted between load balancer and back-end servers.
...
* TCP port 56501 is used for management. If you allocate load balancer public IP addresses from your public VLAN, make sure that any firewalls on that network don't block traffic to the load balancer’s management port or the ports that are used by your application. This action is not required if you choose the IBM system pool option to allocate load balancer public IP addresses.
...
* **Weighted Round Robin** - With this method, the load balancer forwards incoming client connections to the back-end servers in proportion to the weight assigned to these servers. Each server is assigned to a default weight of 50, which you can customize to any value in the range 0 - 100.

	For example, when three application servers A, B and C have weights of 60, 60, and 30, servers A and B receive an equal number of connections. However, server C receives half the number of connections.
...
The load balancer adjusts its capacity automatically according to the load. When this scaling occurs, you might see a change in the number of IP addresses associated with the load balancer's DNS name.

## Multizone region requirements
{: #multi-zone-region-mzr-overview}

A Multizone Region (MZR) helps ensure high availability by deploying your load balancer appliances in multiple data centers. When you create a load balancer, you select the subnet for deployment. If the data center is part of an MZR, one appliance is deployed in your selected data center, and the other is deployed in a different data center within the same region.

For example, Dallas (`us-south`) is an MZR that contains the data centers `dal10`, `dal12`, and `dal13`. Subnet A is in `dal10`, subnets B and C are in `dal12`, and subnets D and E are in `dal13`. If you create a load balancer in the `dal13` data center, the first appliance is deployed in `dal13` and the second appliance is deployed in the subnet with the most available IP addresses between `dal10` or `dal12` data centers.
...
If the selected data center is not part of an MZR, or if VLAN spanning or VRF isn't enabled in your account, the load balancer is created with all nodes that are instantiated in the specified data center. This configuration follows the original deployment behavior.
{: note}

If the selected data center is not part of an MZR, or if VLAN spanning or VRF is not enabled, the load balancer is created only within that data center. It follows the original deployment behavior with all nodes placed there.
